...
//!
//! Implements bitpacked route tokens compatible with the frontier-reapers/starmap specification
//! and its JavaScript reference implementation (`bitpacking.js`).
//!
//! **Waypoint type bit width**
//!
//! The starmap format defines 5 waypoint types (0-4), where type 4 (`SetDestination`)
//! represents gate-to-gate jump sequences. Correct encoding therefore requires **3 bits**
//! for the waypoint type field.
//!
//! Some older C# code used only 2 bits for waypoint type and masked values with `& 0b11`,
//! which silently corrupts type 4 into type 0 during encoding/decoding. This Rust
//! implementation intentionally uses 3 bits to match the starmap specification and
//! JavaScript reference, and may not be compatible with any legacy 2-bit C# encoders.
//!
