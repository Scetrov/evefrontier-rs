...
//!
//! **Note on Format Divergence**: The original C# implementation uses 2 bits for waypoint type,
//! which only supports types 0-3. However, the WaypointType enum defines 5 types (0-4), where
//! type 4 (SetDestination) represents gate-to-gate jump sequences. The C# code masks type values
//! with `& 0b11`, silently corrupting type 4 to type 0 during encoding.
//!
//! This implementation uses **3 bits** for waypoint type to properly support all 5 types.
//! Tokens generated by this implementation may not be compatible with the original C# decoder.
//!
