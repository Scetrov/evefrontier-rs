...
| 0009-kd-tree-spatial-index.md | Precompute and ship K‑D tree spatial index; loader in lib; builder subcommand | Not aligned | No KD-tree module in lib; no builder CLI; no serialization/compression deps | Feature not implemented | Implement KD-tree index per ADR: lib loader/querier; CLI builder; choose format (e.g., `postcard` + `zstd`); tests and docs |
